    ...
    let download_url = ["mediaDownloadUrl", "mediaStreamUrl", "streamLowQualityUrl"]
        ...
        .unwrap_or_else(|| {
            if !hash.is_empty() && !is_folder {
                format!("http://asmr.localhost/file/{}", hash)
            } else {
                String::new()
            }
        });
                        ...
                        let sub_url = ["mediaDownloadUrl", "mediaStreamUrl", "streamUrl"]
                            ...
                            .unwrap_or_else(|| {
                                if !sub_hash.is_empty() {
                                    format!("http://asmr.localhost/file/{}", sub_hash)
                                } else {
                                    String::new()
                                }
                            });
...
fn fetch_from_api(
    ...
    let url = format!("{}/work/{}", API_BASE, rj);
    let mut req = client.get(&url);
    if let Some(t) = token {
        req = req.bearer_auth(t);
    }
...
fn map_work(work: &serde_json::Value, rj: &str) -> ScrapedWork {
    ...
    if let Some(c) = get(work, &["cover", "cover_url", "image"]) {
        w.cover_url = c
            .as_str()
            .map(|s| s.to_string())
    ...
    if let Some(circle) = get(work, &["circle"]) {
        ...
        w.circle_id = get_str(circle, &["id", "circleId"]);
    }
...
mod tests {
    ...
    fn collect_subtitles_matching() {
        ...
        assert_eq!(out[0].subtitles[1].url, "http://asmr.localhost/file/100/3");
    }
}
